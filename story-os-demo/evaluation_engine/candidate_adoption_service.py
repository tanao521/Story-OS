...
from system.safe_write import DataStoreWriteError
...
class CandidateAdoptionService:
    ...
    def __init__(self, context: ProjectContext) -> None:
        ...
        self.version_writer = VersionWriterFacade(context)
        # Preserve test/adapter observability while the writer owns persistence.
        self.version_writer.store = self.store
        self.version_writer.writer.store = self.store

    def _legacy_preview(self, request_id: str) -> dict[str, Any]:
    ...
    def _legacy_adopt(self, request_id: str, payload: dict[str, Any]) -> tuple[dict[str, Any], bool]:
        ...
        if request.get("state") == "adopted":
            if request.get("adoption_operation_id") == operation_id and request.get("adoption_operation_fingerprint", payload.get("__operation_fingerprint", "")) == payload.get("__operation_fingerprint", ""):
            ...
            if request.get("adoption_operation_id") == operation_id:
                raise CandidateAdoptionError("OPERATION_ID_CONFLICT", "operation_id was already used for a different request.")
        ...
        try:
            ...
            request.update({"state": "adopted", "adopted_version_id": new_version["version_id"], "adopted_version": new_version,
                            "adopted_at": _now(), "adoption_operation_id": operation_id, "adoption_operation_fingerprint": str(payload.get("__operation_fingerprint") or ""), "adoption_preview_id": preview["preview_id"],
    ...
    def _legacy_discard(self, request_id: str, payload: dict[str, Any]) -> tuple[dict[str, Any], bool]:
        ...
        if request.get("state") == "discarded":
            if request.get("discard_operation_id") == operation_id and request.get("discard_operation_fingerprint", payload.get("__operation_fingerprint", "")) == payload.get("__operation_fingerprint", ""): return self.improvements.public(request), True
            if request.get("discard_operation_id") == operation_id: raise CandidateAdoptionError("OPERATION_ID_CONFLICT", "operation_id was already used for a different request.")
        ...
        request.update({"state": "discarded", "discarded_at": _now(), "discard_reason": str(payload.get("reason") or "").strip(), "discard_operation_id": operation_id, "discard_operation_fingerprint": str(payload.get("__operation_fingerprint") or ""), "updated_at": _now()})
        self.improvements._save(request)
        try: self._audit("candidate_discarded", request, None, operation_id)
        except DataWriteError: pass
        return self.improvements.public(request), False

    # Compatibility entry points.  The authority is VersionAdoptionService;
    # legacy persistence and response fields stay available to existing callers.
    def preview(self, request_id: str) -> dict[str, Any]:
        from system.version_adoption_service import VersionAdoptionService
        return VersionAdoptionService(self.context, whole_service=self).preview_whole(request_id)

    def adopt(self, request_id: str, payload: dict[str, Any]) -> tuple[dict[str, Any], bool]:
        from system.version_adoption_service import VersionAdoptionService
        return VersionAdoptionService(self.context, whole_service=self).adopt_whole(request_id, payload)

    def discard(self, request_id: str, payload: dict[str, Any]) -> tuple[dict[str, Any], bool]:
        from system.version_adoption_service import VersionAdoptionService
        return VersionAdoptionService(self.context, whole_service=self).discard(request_id, payload)
    ...
    def _create_work_version(self, request: dict[str, Any], candidate: dict[str, Any], source: dict[str, Any], text: str, payload: dict[str, Any], current: dict[str, Any], *, provenance_kind: str = "quality_improvement", provenance: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        base_payload = read_version_payload(current["info"]); now = _now()
        ...
        try:
            revision = RevisionAdapter.candidate(request)
            adoption = AdoptionAdapter.request({**payload, "expected_current_version_revision": current["revision"]})
            result = self.version_writer.create_work_version(
                project=ProjectRef.from_context(self.context), chapter_id=chapter,
                source_type=str(current["source_type"]), source_version=int(current["version"]),
                chapter_title=str(base_payload.get("chapter_title") or ""), candidate_text=text,
                candidate=revision, adoption=adoption, provenance_kind=provenance_kind,
                provenance=details, source_path=str(current["info"].get("json_path") or ""),
                expected_result_hash=str(payload.get("expected_result_content_hash") or "") or None,
            )
        except DataStoreWriteError as exc:
            raise DataWriteError(str(exc)) from exc
        versions = list_versions(chapter, self.context.data_dir)
        info = next(item for item in versions.get("manual", []) if item.get("version_label") == result.version_id)
        return {"version_id": result.version_id, "source_type": "manual", "version": int(info["version"]), "version_label": result.version_id,
                "json_path": info["json_path"], "markdown_path": info["markdown_path"], "parent_version_id": current["version_id"],
                "content_hash": result.new_hash, "revision": int(current["revision"]) + 1, "operation_id": str(payload["operation_id"])}
    ...
    def _rollback_work_version(self, request: dict[str, Any], version: dict[str, Any], old_index: dict[str, Any]) -> None:
        chapter = int(request["chapter_id"])
        try:
            self.version_writer.rollback_work_version(project=ProjectRef.from_context(self.context), chapter_id=chapter, version=version, old_index=old_index, operation_id=str(version.get("operation_id") or "rollback"))
        except (DataStoreWriteError, DataWriteError):
            pass
