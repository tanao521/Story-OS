...
## 重大更新：模拟器模式，让 Story OS 面向每一个人

Story OS 不再只服务于小说作者。最新的**模拟器模式（Simulator Mode）**把系统从“章节写作工作台”扩展成一个可解释、可回放、可人工确认的个人思考与决策工作台：普通人也可以用它整理目标、拆解计划、模拟不同选择、比较方案，并保留每一步的依据与结果。

### 模拟器模式适合谁

- 想规划旅行、学习、健身、搬家、装修、求职或个人项目的人
- 想比较“如果这样做 / 如果那样做”会发生什么的人
- 想把零散想法整理成清晰步骤，同时保留自己的最终决定的人
- 不写小说，但希望拥有一个本地、可追溯、不会擅自替自己做决定的 AI 工作台的人

### 它能做什么

在模拟器模式中，你可以把一个现实问题拆成场景、目标、约束、候选行动和预期结果，逐步运行不同方案并比较差异。系统会展示上下文、状态、证据、风险和下一步建议；你可以随时暂停、回看、修改输入或重新运行。模拟结果是辅助信息，不会被冒充为事实，也不会自动替你执行外部操作。

典型例子：

```text
旅行：比较“预算优先”和“体验优先”的两套行程
学习：模拟每天 30 分钟、60 分钟和周末集中学习的进度差异
求职：比较不同岗位选择对技能、时间和收入目标的影响
健身：根据时间、器械和身体限制，比较不同训练计划
个人项目：把一个模糊目标拆成里程碑、依赖关系和可执行任务
```

### 与小说模式的关系

小说模式仍然提供章节规划、草稿、版本、质量检查、人工审核和正史提交；模拟器模式则使用独立的模拟状态与上下文，不会把模拟结果自动写入小说正文、正史、项目设定或章节进度。两种模式共享一致的安全边界：重要改变必须由人明确确认，系统不会因为一次模拟就自动提交、发布或改写真实资料。
...
打开 `http://127.0.0.1:7860` 后，在顶部工作模式切换中选择“模拟器”。如果你只是想探索一个问题，不需要先创建小说；创建一个模拟场景，输入目标与约束，就可以开始比较方案。

### 设计原则

- **人拥有最终决定权**：模拟、建议和评分都不是自动批准
- **本地优先**：默认使用本地项目数据，不要求把私人内容上传到平台
- **可解释、可回放**：每次运行都绑定输入、状态和结果，便于复盘
- **边界清晰**：模拟结果、建议、缓存和真实事实分开保存
- **渐进式使用**：可以只用规划与比较，也可以继续接入小说创作工作流

这次更新的目标，是让 Story OS 成为“任何人都能用来把复杂事情想清楚”的系统，而不仅是一套小说生产工具。
...
- 叙事调度保存在 `data/planning_control/narrative_schedules.json`：作者可把已有主线、角色弧线或伏笔手动安排到未来章节槽位，查看主体时间线、槽位负载与依赖警告；不会自动移动章节、修改正史或调用模型。
...
- `GET/POST/PUT /api/planning-control/schedules`，以及 `/health`、`/validate`、`/timeline`、`/by-slot` 和状态转换子资源
