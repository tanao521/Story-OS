...
let memoryRepairRequest = 0;
async function runMemoryHealth(full=false){const t=document.getElementById("memoryHealthOutput"),ticket=++memoryRepairRequest;if(t)t.textContent="\u6b63\u5728\u68c0\u67e5\u8bb0\u5fc6\u5065\u5eb7\u2026";try{const d=await apiGet("/api/memory-health"+(full?"?full=true":""));if(ticket!==memoryRepairRequest)return;if(!d.ok){if(t)t.textContent="\u8bb0\u5fc6\u5065\u5eb7\u68c0\u67e5\u5931\u8d25\u3002";logApiResult("\u8bb0\u5fc6\u5065\u5eb7",d);return;}renderMemoryHealth(d.result||{});await loadMemoryRepairStatus(ticket);logApiResult("\u8bb0\u5fc6\u5065\u5eb7",d);await refreshStatus();}catch(e){if(ticket!==memoryRepairRequest)return;if(t)t.textContent="\u8bb0\u5fc6\u5065\u5eb7\u68c0\u67e5\u5931\u8d25\u3002";logMessage("\u8bb0\u5fc6\u5065\u5eb7\u68c0\u67e5\u5931\u8d25\u3002","error");}}
function renderMemoryHealth(report){const t=document.getElementById("memoryHealthOutput");if(!t)return;const s=report.summary||{},issues=Array.isArray(report.issues)?report.issues:[],suggestions=Array.isArray(report.suggestions)?report.suggestions:[];const statusText=report.overall_status==="ok"?"\u8bb0\u5fc6\u72b6\u6001\u826f\u597d\u3002":report.overall_status==="warning"?"\u5b58\u5728\u9700\u8981\u5904\u7406\u7684\u9879\u76ee\uff0c\u4f46\u901a\u5e38\u4e0d\u5f71\u54cd\u7ee7\u7eed\u521b\u4f5c\u3002":"\u5b58\u5728\u9700\u8981\u5148\u5904\u7406\u7684\u4e00\u81f4\u6027\u95ee\u9898\u3002";t.classList.remove("empty-state");t.innerHTML="<div class=\"health-summary\"><div class=\"health-status "+healthLevelClass(report.overall_status)+"\">"+escapeHtml(healthStatusLabel(report.overall_status))+"</div><div class=\"health-score\">\u5f97\u5206\uff1a"+escapeHtml(formatScore(report.overall_score))+"</div><div>\u9519\u8bef\uff1a"+Number(s.errors||0)+"</div><div>\u8b66\u544a\uff1a"+Number(s.warnings||0)+"</div><div>\u63d0\u793a\uff1a"+Number(s.infos||0)+"</div></div><p class=\"health-suggestion\">"+escapeHtml(statusText)+"</p><h3>\u95ee\u9898\u660e\u7ec6</h3><div class=\"health-issue-list\">"+(issues.length?issues.map(renderHealthIssue).join(""):"<div class=\"empty-state\">\u672a\u53d1\u73b0\u95ee\u9898\u3002</div>")+"</div><h3>\u5efa\u8bae</h3><ul class=\"health-suggestion-list\">"+(suggestions.length?suggestions.map(x=>"<li>"+escapeHtml(String(x))+"</li>").join(""):"<li>\u8bf7\u4fdd\u6301\u9010\u7ae0\u521b\u4f5c\u4e0e\u4eba\u5de5\u5ba1\u6838\u6d41\u7a0b\u3002</li>")+"</ul><div id=\"memoryRepairStatus\" class=\"health-repair\"></div>";}
function renderHealthIssue(issue){const action=issue.recoverable&&issue.repair_action?(issue.repair_action==="generate_quality_report"?"<button class=\"btn btn-primary btn-compact\" type=\"button\" onclick=\"startMemoryRepair('quality')\">\u751f\u6210\u5f53\u524d\u6b63\u53f2\u62a5\u544a</button>":"<button class=\"btn btn-secondary btn-compact\" type=\"button\" onclick=\"startMemoryRepair('vector')\">\u521d\u59cb\u5316\u672c\u5730\u7d22\u5f15</button>"):"";return "<article class=\"health-issue "+healthLevelClass(issue.level||"info")+"\"><strong>"+escapeHtml(issue.id||"\u95ee\u9898")+"</strong><span class=\"health-category\">"+escapeHtml(issue.category||"")+"</span><p>"+escapeHtml(issue.message||"")+"</p><code class=\"health-path\">"+escapeHtml(issue.path||"")+"</code><p class=\"health-suggestion\">"+escapeHtml(issue.impact||issue.suggested_action||"")+"</p>"+action+"</article>";}
function healthStatusLabel(status){if(status==="ok")return "\u5065\u5eb7";if(status==="warning")return "\u9700\u5173\u6ce8";if(status==="error")return "\u5f85\u5904\u7406";return "\u672a\u77e5";}
async function loadMemoryRepairStatus(ticket=memoryRepairRequest){try{const results=await Promise.all([apiGet("/api/quality-reports/status"),apiGet("/api/vector-index/status")]);if(ticket!==memoryRepairRequest)return;const target=document.getElementById("memoryRepairStatus");if(!target)return;const quality=(results[0].result||{}),vector=(results[1].result||{});target.innerHTML="<h3>\u53ef\u6062\u590d\u9879</h3><div class=\"health-issue-list\"><article class=\"health-issue\"><strong>\u5f53\u524d\u6b63\u53f2\u8d28\u91cf\u62a5\u544a</strong><p>"+escapeHtml(quality.message||"\u6b63\u5728\u8bfb\u53d6\u72b6\u6001\u3002")+"</p>"+(quality.repair_available?"<button class=\"btn btn-primary btn-compact\" type=\"button\" onclick=\"startMemoryRepair('quality')\">\u751f\u6210\u62a5\u544a</button>":"")+"</article><article class=\"health-issue\"><strong>\u672c\u5730\u5411\u91cf\u7d22\u5f15</strong><p>"+escapeHtml(vector.message||"\u6b63\u5728\u8bfb\u53d6\u72b6\u6001\u3002")+"</p>"+(vector.repair_available?"<button class=\"btn btn-secondary btn-compact\" type=\"button\" onclick=\"startMemoryRepair('vector')\">\u521d\u59cb\u5316\u7d22\u5f15</button>":"")+"</article></div>";}catch(error){if(ticket===memoryRepairRequest){const target=document.getElementById("memoryRepairStatus");if(target)target.textContent="\u65e0\u6cd5\u8bfb\u53d6\u53ef\u6062\u590d\u9879\u72b6\u6001\u3002";}}}
async function startMemoryRepair(kind){const target=document.getElementById("memoryRepairStatus");if(target)target.textContent="\u6b63\u5728\u521b\u5efa\u4efb\u52a1\u2026";try{const data=await apiPost(kind==="quality"?"/api/quality-reports/repair":"/api/vector-index/initialize",{});if(!data.ok)throw new Error(data.message||"\u521b\u5efa\u4efb\u52a1\u5931\u8d25\u3002");const job=(data.result||{}).job||{};if(target)target.textContent="\u5df2\u521b\u5efa\u4efb\u52a1\uff1a"+(job.job_id||"\u6b63\u5728\u6392\u961f")+"\u3002";if(job.job_id)pollMemoryRepairJob(job.job_id,0);else runMemoryHealth(false);}catch(error){if(target)target.textContent="\u4efb\u52a1\u521b\u5efa\u5931\u8d25\uff1a"+(error.message||"\u672a\u77e5\u9519\u8bef");}}
async function pollMemoryRepairJob(jobId,attempt){try{const data=await apiGet("/api/jobs/"+encodeURIComponent(jobId));const job=(data.result||{}).job||{};if(["completed","completed_with_warnings","failed","cancelled","recoverable_failed"].includes(job.status)){runMemoryHealth(false);return;}if(attempt<60)setTimeout(()=>pollMemoryRepairJob(jobId,attempt+1),1000);}catch(error){if(attempt<6)setTimeout(()=>pollMemoryRepairJob(jobId,attempt+1),1500);}}
...
function showProjectCenter(){["setup-view","dashboard-view","logs-view"].forEach(id=>document.getElementById(id)?.classList.add("hidden"));document.getElementById("project-center-view")?.classList.remove("hidden");loadProjectCenter();}
...
const stage3Create= createStoryProject;createStoryProject=async function(){const e=document.getElementById("setup-error"),title=valueOf("setup-title");e.textContent="";if(!title){e.textContent="A novel title is required.";return;}const b={title:title,genre:valueOf("setup-genre"),custom_genre:valueOf("setup-custom-genre"),length_type:valueOf("setup-length"),target_word_count:Number(valueOf("setup-word-count")||0),world_style:valueOf("setup-world-style"),tone:valueOf("setup-tone"),writing_style:valueOf("setup-writing-style"),narration:valueOf("setup-narration"),character_structure:valueOf("setup-character-structure"),romance_level:valueOf("setup-romance"),focus:listValueOf("setup-focus"),avoid:listValueOf("setup-avoid"),anti_ai_style_rules:listValueOf("setup-anti-ai"),need_outline:document.getElementById("setup-need-outline").checked,use_deepseek:document.getElementById("setup-use-deepseek").checked};const r=await apiPost("/api/projects",b);if(!r.ok){e.textContent=(r.errors||[]).join(" ")||r.message;return;}showDashboard();await refreshAll();};


/* Stage 4: persistent operation center. This deliberately polls only job APIs. */
const storyJobCenter = { jobId: null, timer: null, failures: 0, history: [] };
...
function setJobButtonsBusy(busy) { document.querySelectorAll("[data-workbench-generate]").forEach((button) => { button.disabled = busy; }); }
function renderJobCenter(job) {
  const current = document.getElementById("job-current-card"), steps = document.getElementById("job-steps"), logs = document.getElementById("job-log-output");
  if (!current || !steps || !logs) return;
  if (!job) { current.textContent = "No active task"; steps.innerHTML = '<li class="empty-state">Select a task to view its steps.</li>'; logs.textContent = "No task logs"; return; }
  const progress = job.progress || {}, step = job.current_step || "-";
  current.innerHTML = `<strong>${jobEscape(storyJobNames[job.job_type] || job.job_type)}</strong><br><span class="job-status">${jobStatusLabel(job.status)} | ${jobEscape(step)} | ${Number(progress.current || 0)} / ${Number(progress.total || 0) || "-"}</span><p>${jobEscape(job.message || "")}</p><div class="job-actions">${["queued","running","cancel_requested"].includes(job.status) ? `<button class="btn btn-danger btn-compact" type="button" onclick="cancelStoryJob('${jobEscape(job.job_id)}')">Cancel</button>` : ""}${["failed","cancelled","interrupted","completed_with_warnings"].includes(job.status) ? `<button class="btn btn-secondary btn-compact" type="button" onclick="retryStoryJob('${jobEscape(job.job_id)}')">Retry</button>` : ""}</div>`;
  const jobSteps = Array.isArray(job.steps) ? job.steps : [];
  steps.innerHTML = jobSteps.length ? jobSteps.map((item) => `<li class="job-step-${jobEscape(item.status)}"><span class="job-step-icon">${jobStepIcon(item.status)}</span><span><strong>${jobEscape(item.label || item.name)}</strong><br><small>${jobEscape(item.message || item.status)}</small></span></li>`).join("") : '<li class="empty-state">The task has not entered a step yet.</li>';
  const entries = Array.isArray(job.logs) ? job.logs : [];
  logs.textContent = entries.length ? entries.map((entry) => `[${entry.timestamp || ""}] ${entry.step || "job"}: ${entry.message || ""}`).join("\n") : "No task logs";
}
function renderJobHistory(jobs) {
  const target = document.getElementById("job-history"); if (!target) return;
  storyJobCenter.history = Array.isArray(jobs) ? jobs : [];
  if (!storyJobCenter.history.length) { target.textContent = "No task history"; return; }
  target.innerHTML = storyJobCenter.history.map((job) => `<article class="job-history-item"><span><strong>${jobEscape(storyJobNames[job.job_type] || job.job_type)}</strong><br><small>${jobEscape(job.created_at || "")}</small></span><span class="job-actions"><span class="job-status">${jobStatusLabel(job.status)}</span><button class="btn btn-secondary btn-compact" type="button" onclick="openStoryJob('${jobEscape(job.job_id)}')">Details</button>${["failed","cancelled","interrupted","completed_with_warnings"].includes(job.status) ? `<button class="btn btn-secondary btn-compact" type="button" onclick="retryStoryJob('${jobEscape(job.job_id)}')">Retry</button>` : ""}</span></article>`).join("");
}
async function refreshJobHistory() { const response = await apiGet("/api/jobs?limit=20"); if (response.ok === false) throw new Error(response.message || "Could not load task history"); renderJobHistory(response.result?.jobs || []); }
async function openStoryJob(jobId) { storyJobCenter.jobId = jobId; stopJobPolling(); await pollStoryJob(); }
async function refreshJobCenter() { try { const active = await apiGet("/api/jobs/active"), jobs = active.result?.jobs || []; if (jobs[0]) { storyJobCenter.jobId = jobs[0].job_id; await pollStoryJob(); } else { renderJobCenter(null); await refreshJobHistory(); } } catch (error) { logMessage(error.message || "Task center connection issue", "warning"); } }
async function pollStoryJob() { if (!storyJobCenter.jobId) return; try { const response = await apiGet(`/api/jobs/${encodeURIComponent(storyJobCenter.jobId)}`); if (response.ok === false) throw new Error(response.message || "Could not read task"); const job = response.result?.job; renderJobCenter(job); storyJobCenter.failures = 0; setJobButtonsBusy(["queued","running","cancel_requested"].includes(job?.status)); if (job && !storyJobTerminal.has(job.status)) { storyJobCenter.timer = window.setTimeout(pollStoryJob, document.hidden ? 5000 : 1500); return; } stopJobPolling(); setJobButtonsBusy(false); await refreshJobHistory(); if (job && ["completed","completed_with_warnings","waiting_for_review"].includes(job.status)) await refreshAll(); } catch (error) { storyJobCenter.failures += 1; const delay = Math.min(15000, 1000 * (2 ** storyJobCenter.failures)); logMessage(error.message || "Task polling connection issue", "warning"); if (storyJobCenter.failures < 5) storyJobCenter.timer = window.setTimeout(pollStoryJob, delay); } }
...
const stage4OpenProject = window.openProject;
