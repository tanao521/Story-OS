...
def _map_domain_error(exc: NarrativeTurnError) -> _RequestError:
    ...
    safe_messages = {
        ...
        NarrativeTurnError.TURN_ALREADY_CONFIRMED: ("TURN_ALREADY_CONFIRMED", "该回合已被其他操作确认。", 409),
        ...
        NarrativeTurnError.CONFIRM_RECOVERY_REQUIRED: ("CONFIRM_RECOVERY_REQUIRED", "确认记录需要人工核对后再恢复。", 409),
        NarrativeTurnError.CONTEXT_STALE: ("CONTEXT_STALE", "上下文已变化，请重新加载。", 409),
        NarrativeTurnError.VALIDATION_STALE: ("VALIDATION_STALE", "可行性校验已过期，请重新校验。", 409),
        NarrativeTurnError.PREVIEW_STALE: ("PREVIEW_STALE", "后果预览已过期，请重新预览。", 409),
