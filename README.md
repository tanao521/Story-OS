...
Story OS 是一个**开源的、本地优先的**长篇小说 AI 创作系统。

它的定位不是"一键生成小说"的玩具，而是一个为严肃长篇创作者设计的**工程化写作平台**：把 AI 作为创作流水线中的一个工具节点，而不是把整个创作过程黑盒化。现在增加了重大更新，为了保证普通用户的体验，防止用户人工审核时感到枯燥乏味，Story OS 引入了叙事回合制交互模式，把章节创作从"AI 一次性写完" 变成"作者选择行动 → 系统推演结果 → 再选择下一个行动"的可控流程。

### 核心设计哲学

- **本地优先** — 所有数据默认保存在本地，不依赖云端服务
- **人工可控** — AI 的每一步输出都经过人工审核闸门
- **长期记忆** — Obsidian + Chroma 双记忆系统，支持超长篇一致性
- **可追溯性** — 完整的版本历史、操作日志和状态转换链
- **事务安全** — 关键操作支持幂等重放、并发仲裁和故障恢复
- **渐进式增强** — 没有模型也能跑通主流程，模型只是加速器

### 适合谁用

- 写 20 万字以上长篇小说的作者
- 担心 AI 上下文不够长、前后矛盾的创作者
- 喜欢工程化方法管理创作流程的人
- 想研究 AI Agent 工作流的开发者
...
### 8. Narrative Turn 叙事回合系统

Story OS 引入了**叙事回合制**交互模式，把章节创作从"AI 一次性写完"
变成"作者选择行动 → 系统推演结果 → 再选择下一个行动"的可控流程。

核心特性：

- **推荐行动** — 系统根据当前情境生成 3 个推荐行动，每个都有可行性分析
- **自定义行动** — 作者可以输入自定义行动，系统会进行可行性验证
- **可行性分级** — `allowed` / `allowed_with_cost` / `blocked` / `requires_clarification`
- **定性预览** — 确认前可以预览行动的预期后果和风险
- **事务安全** — 确认操作支持幂等重放、并发仲裁和故障恢复
- **不可变记录** — 每个 Turn 的计划、验证、结果、过渡链都是 append-only 的

---

### 9. 多分支时间线

支持在同一时间线上创建多个分支，隔离不同的剧情走向：

- **分支隔离** — 每个分支有独立的叙事状态和事件日志
- **状态投影** — 分支本地状态通过 CAS 原子更新，不会丢失更新
- **事件溯源** — append-only 事件日志，支持回放和审计
- **生命周期管理** — 分支有 open / archived 状态管理

分支系统确保你可以大胆尝试不同剧情走向，而不影响主线故事。
...
| 上下文构建 | 最近章节 + 摘要记忆 |
| 章节规划 | 只规划下一章 |
| Narrative Turn | 叙事回合制交互（推荐行动 + 自定义行动） |
| 分支系统 | 多分支时间线，支持分支隔离和状态投影 |
...
1. 生成故事蓝图：由规划层建立主线、核心冲突、故事阶段和初始伏笔池。
2. 生成角色档案与世界观：角色档案会写入 data/characters.json，并回写蓝图的 character_bible。
3. 生成下一章规划：把蓝图、角色档案、世界规则、已开伏笔、章节摘要和向量记忆一起作为约束，生成 data/next_chapter_plan.json。
4. 生成正文：正文生成只读取当前章节计划，不会跳过人工审核直接推进章节。
